#!/usr/bin/env bash
# Export LFD (TG 46038) calls from production and pull to local /tmp/tlr-debug/tlr-lfd-export
set -euo pipefail

LIMIT="${1:-600}"
LOCAL_DIR="${LOCAL_DIR:-/tmp/tlr-debug/tlr-lfd-export}"
REMOTE_DIR="/tmp/tlr-debug/tlr-lfd-export"
...
echo "Exporting $LIMIT calls on production..."
ssh "$PROD_HOST" "chmod +x /tmp/export-lfd-calls && /tmp/export-lfd-calls -ini '$PROD_INI' -out '$REMOTE_DIR' -limit $LIMIT"
